#' Perform Sensitivity Analysis Using Flux Balance Analysis (FBA)
#'
#' This function performs a global sensitivity analysis using FBA simulations.
#' It evaluates the impact of model parameter variations on a predefined reaction objective.
#' The process involves Sobol' sampling, FBA runs in parallel, and computation of Sobol' indices
#' with bootstrapped confidence intervals.
#'
#' @param result_dir Directory to save results and intermediate files.
#' @param model_file Path to the metabolic model file.
#' @param all_react_file RDS file containing reaction annotations.
#' @param param_values_file File path to save the sampled parameter matrix.
#' @param bounds_set_file RDS file containing bounds for parameters.
#' @param fbasol_t_file Path to temporary FBA solution file.
#' @param fbasol_file Path to final FBA solution file.
#' @param Y_file File to save outputs from first objective.
#' @param Y2_file File to save outputs from second objective.
#' @param indices_file File to save computed sensitivity indices.
#' @param Y2_reaction ID of the reaction associated with second objective.
#' @param N Number of samples (e.g., 2^13).
#' @param cores Number of cores for parallel computing.
#' @param b1 Lower bound for sampling parameters.
#' @param b2 Upper bound for sampling parameters.
#'
#' @return Saves RDS files of sensitivity indices, parameter values, and FBA solutions.
#' @export
#'
#' @examples
#' \dontrun{
#' SA_FBA(result_dir = "results/",
#'        model_file = "model.sbml",
#'        all_react_file = "reactions.rds",
#'        param_values_file = "params.rds",
#'        bounds_set_file = "bounds.rds",
#'        fbasol_t_file = "tmp_sol.rds",
#'        fbasol_file = "sol.rds",
#'        Y_file = "Y.rds",
#'        Y2_file = "Y2.rds",
#'        indices_file = "indices.rds",
#'        Y2_reaction = "biomass205",
#'        N = 8192,
#'        cores = 4,
#'        b1 = -10.0,
#'        b2 = 0.0)
#' }
#' @import stats dplyr tidyr parallel
...
#' Generate Stoichiometric Reaction Equations from a Metabolic Model
#'
#' This function constructs textual representations of reaction equations
#' from a metabolic model object. It considers reversibility, compartment annotations,
#' and can optionally create a closed network by adding boundary metabolites.
#'
#' @param model A metabolic model object with stoichiometric matrix \code{S},
#'              metabolite IDs, and reaction bounds.
#' @param makeClosedNetwork Logical. If \code{TRUE}, exchange reactions are made closed
#'                          by adding boundary metabolites.
#' @param entrydelim Character delimiter used when concatenating compartment annotations.
#'                   Defaults to ", ".
#' @param extMetFlag Character flag used to mark external/boundary metabolites. Defaults to "b".
#'
#' @return A list with the following elements:
#' \describe{
#'   \item{equat}{Character vector of reaction equations.}
#'   \item{compat}{Character vector of compartment labels per reaction.}
#'   \item{revers}{Character vector indicating reversibility ("Reversible" or "Irreversible").}
#'   \item{metab}{Vector of cleaned metabolite IDs without compartment suffixes.}
#' }
#'
#' @examples
#' \dontrun{
#' rxn_equations <- GenerateReactionEquations(model = my_model, makeClosedNetwork = TRUE)
#' head(rxn_equations$equat)
#' }
