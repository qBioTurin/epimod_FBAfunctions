...
# ---------------------------------------------------------------------------
# Build a general-purpose list of FBA-compatible biounits  (minimal fix)
# ---------------------------------------------------------------------------
#' @export
make_biounit_models <- function(model_names,
  ...
  lapply(seq_along(model_names), function(i) {
    ...
    mn_full <- model_names[i]                                   # can be a full path
    mn_base <- tools::file_path_sans_ext(basename(mn_full))     # just “Model_Name”
    
    abbrs <- derive_abbrs(mn_base)                              # ← the only change
    
    list(
      FBAmodel            = mn_full,        # leave as-is
      unit                = gsub("_", " ", mn_base),
      abbreviation        = abbrs,
